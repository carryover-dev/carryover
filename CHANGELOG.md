...
- Progress log now resets on new session instead of accumulating unrelated history. A `<!-- session: <uuid> -->` watermark on line 1 detects session boundaries; a one-liner `_Previous session (date): task_` is preserved for context.
- Hook events cache the real `project_dir` into the `"default"` cursor so fs-watcher events always write to the correct project directory instead of falling back to slug decoding or home dir.
- `carryoverd status` message now correctly shows `carryoverd install` instead of `carryover install`.

### Added

- `carryoverd start` subcommand for explicit daemon startup (macOS foreground use).
